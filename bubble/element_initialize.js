...
function (instance, context) {
  ...
  if (!window.GWMoodboard) {
    // The bundle is loaded by a <script> tag in the element's Headers, which runs before
    // this. If it's missing, the header is wrong or the file didn't load — say so on the
    // element rather than failing silently to an empty box.
    el.innerHTML =
      '<div style="padding:16px;font:13px system-ui;color:#b04a3a">' +
      'Moodboard failed to load: gw-moodboard.js is missing. Check the element Headers.' +
      '</div>';
    return;
  }
