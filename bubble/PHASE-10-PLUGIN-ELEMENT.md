...
| Collaborator names | `collaborator_names` | Dynamic value / text, **list** | — | `…Collaborator Accesses:each item's User's First Last` |
...
**Every one of these binds to the User, not to the Collaborator Access.** The first pass
bound `collaborator_ids` to `:each item's unique id` — the access record's id, not the
person's — and comments store `Created By`, a User. Nothing matched, and the failure is
silent: no author names, no photos, no error. Same trap on `collaborator_photos`, bound to
`First Last`, which puts a name where a URL belongs.
